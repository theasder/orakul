...
describe('RESEARCH-AND-PLAN', () => {
  ...
  test('the recurring defect class is recorded with real cases', () => {
    ...
    // третьем. Дописав восьмой случай, я оставил «нашлась семь раз» — ровно
    ...
    // остальных разделов, и счёт вышел 48 вместо восьми.
    // Берётся ПЕРВАЯ таблица раздела и только она: `section` тянется до конца
    // документа, и без границы сюда попадали чужие таблицы — счёт выходил 48,
    // а с границей по заголовку 11, потому что в этом же разделе есть вторая
