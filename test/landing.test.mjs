...
describe('orakul landing (ru)', () => {
  ...
  test('the speed on the page is the whole wait, not one step of it', () => {
    ...
    const claim = /(\d+) тысяч слов, \d+ тысяч реплик — отвечает[^:]*: на нашей машине ([\d,]+) секунды/
      .exec(text);
    ...
    // Потолок в 12 секунд не ловит занижение: 0,4 меньше двенадцати так же,
    // как 2,3. Зато половины должны складываться в целое — это ловит правку
    // одного числа из трёх, самую вероятную.
    const parts = /из них ([\d,]+) на разбор слов и ([\d,]+) на сам поиск/.exec(text);
    assert.ok(parts, 'страница больше не разбивает ожидание на половины');
    const num = (s) => Number(s.replace(',', '.'));
    const sum = num(parts[1]) + num(parts[2]);
    assert.ok(Math.abs(sum - num(seconds)) <= 0.1,
      `половины дают ${sum.toFixed(2)} с, а целое названо ${seconds} с`);
  });
