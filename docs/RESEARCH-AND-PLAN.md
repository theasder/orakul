...
| WEEEK | нет | не нашли описания параметров выдачи задач |
...
`unreadable`. Разница не косметическая: первое чинится в настройках, второе —
новым токеном, третье — ожиданием, четвёртое означает, что сервис сменил формат
и коннектор пора править.
...
**Найдено и НЕ закрыто:** `FirefliesPastCalls.parseMeetingList` возвращает
пустой список и когда разобрать ответ не вышло, и когда встреч правда нет.
Через `MCPContextImport.firefliesRecentMeetings` это показывается человеку как
«прошлых звонков нет» — при смене схемы у Fireflies или при ответе-ошибке он
решит, что импортировать нечего.

Почему не закрыто сразу: у разбора девять мест в тестах с необязательным
возвратом, и правка требует либо смены сигнатуры везде, либо второго метода.
Решение — отдельный разбор, возвращающий `nil` на непонятый ответ, и вызов
из импорта через него; существующие проверки остаются на прежнем. Это
следующая правка, а не сделанная.
...
## Приложение задач в трекер: что позволяют API (проверено 2026-08-11)
