...
системных запросов, свой словарь. 3225 тестов проходят.
...
Падежи в поиске: вопрос «что решили по развёртыванию» находит разговор, где
сказали «развёртывание». Разряд слов на «-ние» — обновление, подключение,
решение, тестирование — не находился ни в одном падеже, кроме именительного;
разбор чинился 14 августа, подробности в `docs/RESEARCH-AND-PLAN.md`, §6.7.
Чего разбор не умеет, там же: глагол через вид не сводится, «выкатываем» не
найдётся по «выкатить».
...
Тесты: `cd app && swift test` (2679 штук), mvp — 355 и `npm test` в корне (191 — про
