...
Если у вас уже стоит whisper.cpp, расшифровка тоже работает — orakul запускает
вашу же программу и подставляет файл вместо `{файл}`:

```swift
let engine = ExternalTranscriber(
    command: "whisper-cli -m ~/models/ggml-large-v3.bin -l ru -otxt -f {файл}")
```
