...
struct AudioAccumulatorTests {
    ...
    func silenceIsDetected() {
        ...
        // Уровень настоящего фона тихой комнаты, измеренный на этой машине:
        // RMS около 0.00114. Раньше он проходил порог, то есть «тишина» ловилась
        // только на мёртвом канале.
        quiet.append((0..<32_000).map { _ in Float.random(in: -0.002...0.002) })
    ...
    func levelIsAveraged() {
        ...
        //
        // Порог 0.005 откалиброван замером: фон тихой комнаты на реальном
        // микрофоне — RMS 0.00114. Один щелчок на две секунды даёт около
        // 0.0056 и порог всё ещё переживает; это осознанный остаток, потому
        // что поднимать порог дальше значит подходить к уровню тихой речи.
        ...
        #expect(!click.looksSilent, "0.0056 всё ещё выше порога 0.005 — задокументировано выше")
    ...
    @Test("порог тишины выше измеренного фона реальной комнаты")
    func thresholdIsAboveMeasuredRoomNoise() {
        // Число из замера, а не из головы: 0.00114 — фон тихого кабинета,
        // записанный этим же приложением. Порог обязан быть выше, иначе
        // «тишина» означает лишь «микрофон отключён».
        #expect(AudioAccumulator.silenceThreshold > 0.00114,
                "порог ниже фона — тишина не будет определяться никогда")
        // И заметно ниже речи, иначе начнём отказывать настоящим созвонам.
        #expect(AudioAccumulator.silenceThreshold < 0.02)
    }

    @Test("уровень доступен наружу, а не только вердикт")
    func levelIsExposed() {
        // Человеку показывают число: «тишина» без уровня неотличима от
        // сломанного преобразования, и разбираться пришлось бы вслепую.
        var buffer = AudioAccumulator()
        buffer.append(tone(seconds: 1))
        #expect(buffer.level > 0.1)
        #expect(AudioAccumulator().level == 0)
    }
