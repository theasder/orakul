...
/// The measurement F5 ships dark waiting for.
///
/// `LocalDiarization.isEnabled` stays false until the on-device pass is known
/// to be right, because a confidently wrong speaker label is the complaint it
/// answers. This harness is how that gets decided: point it at recordings whose
/// speaker count is known and see whether the model agrees.
///
///     CRUXWING_DIARIZE_DIR=/path/with/wavs \
///     CRUXWING_DIARIZE_EXPECT=2 \
...
struct LocalDiarizationMeasurement {
    ...
    func measureAgainstKnownRecordings() async throws {
        ...
        if env["CRUXWING_DIARIZE_SWEEP"] != nil {
            for threshold in [Float(0.5), 0.6, 0.65, 0.7, 0.8, 0.9] {
                var hits = 0
                var counts: [Int] = []
                for name in files {
                    let wav = try Data(contentsOf: URL(fileURLWithPath: dir).appendingPathComponent(name))
                    ...
                    let segments = try await LocalDiarization.segments(
                        samples: samples, clusteringThreshold: threshold)
                    let found = SpeakerAssignment.distinctSpeakers(in: segments)
                    counts.append(found)
    ...
    ///
    ///     CRUXWING_DIARIZE_GT_DIR=/path/with/wav+gt \
    ...
    func measureAttributionAgainstGroundTruth() async throws {
        ...
        let thresholds = (env["CRUXWING_DIARIZE_THRESHOLDS"] ?? "0.5,0.6,0.65,0.7,0.8,0.9")
            .split(separator: ",").compactMap { Float($0) }

        var meanByThreshold: [(threshold: Float, mean: Double, asRead: Double, exactCounts: Int)] = []
        for threshold in thresholds {
            ...
            for stem in stems {
                ...
                if env["CRUXWING_DIARIZE_DIAG"] != nil {
