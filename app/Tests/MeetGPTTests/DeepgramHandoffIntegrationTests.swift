...
private actor DelayedHandoffTranscriber: TranscriptionService {
    ...
    func release(_ answers: [String]) {
        ...
        for (ordinal, continuation) in pending {
            continuation.resume(returning: answers[ordinal - 1])
        }
    }
...
struct DeepgramHandoffIntegrationTests {
    ...
    func successfulSwitchExecutesProductionRoutes() async {
    ...
    func unavailableEngineStopsAtAvailabilityGate() {
        ...
        #expect(!state.selectTranscriptionEngine(.deepgram))
        #expect(state.selectedTranscriptionEngine == .local)
        #expect(state.lastError?.contains("not available") == true)
    }

    @Test("delayed Local chunks and the partial accumulator survive a Local to Instant handoff once")
    func delayedLocalDrainSurvivesInstantHandoff() async {
        let savedEngine = Config.transcriptionEngineValue
        defer { Config.transcriptionEngineValue = savedEngine }
        Config.transcriptionEngineValue = .local
        ...
        #expect(text.firstIndex(of: "local full boundary")! < text.firstIndex(of: "future instant words")!)
        #expect(text.firstIndex(of: "local partial boundary")! < text.firstIndex(of: "future instant words")!)
    ...
    func instantTailSurvivesChunkedHandoff() async {
        let savedEngine = Config.transcriptionEngineValue
        defer { Config.transcriptionEngineValue = savedEngine }
        Config.transcriptionEngineValue = .local
        ...
        #expect(ordered.firstIndex(of: "trailing instant final")!
                < ordered.firstIndex(of: "future chunked words")!)
    }

    @Test("pre-ready Instant failure restores the prior transcriber and routes")
    func preReadyFailureExecutesProductionRollback() async {
        let savedEngine = Config.transcriptionEngineValue
        defer { Config.transcriptionEngineValue = savedEngine }
        Config.transcriptionEngineValue = .local
        ...
        #expect(await waitFor {
            state.liveTranscriptionConfiguration().active?.engine == .local
                && state.lastError?.contains("Continued with Private") == true
        })
        #expect(state.selectedTranscriptionEngine == .local)
    ...
    func simultaneousPreReadyFailuresReplayStartupAudioExactlyOnce() async {
        let savedEngine = Config.transcriptionEngineValue
        defer { Config.transcriptionEngineValue = savedEngine }
        Config.transcriptionEngineValue = .local
