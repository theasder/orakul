...
struct NoTariffsTests {
    ...
    @Test("доступны все модели, а не четыре из четырнадцати")
    func everyModelIsAvailable() {
        // На бесплатном уровне Cruxwing открыты четыре модели из четырнадцати.
        // Российскому разработчику это выглядело бы как «остальное за деньги»,
        // которых взять неоткуда: платить orakul не за что.
        ...
        // Числа в названии этой проверки и на странице («открыты все, а не
        // четыре из четырнадцати») — не украшение: они говорят, от чего мы
        // отказались. Раньше здесь стояло «две из тринадцати» — оба числа
        // устарели молча, потому что их ничто не держало.
        let onFree = LLMCatalog.all.filter { $0.isAvailable(for: .free) }.count
        #expect(onFree == 4 && LLMCatalog.all.count == 14, """
            каталог изменился: на бесплатном уровне \(onFree) из \(LLMCatalog.all.count).             Поправьте число здесь, в названии этой проверки и на странице, в разделе             «Сколько стоит».
            """)
    }
