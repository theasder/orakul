...
final class DeepgramHandoffState: @unchecked Sendable {
    ...
    static let readinessTimeoutNanoseconds: UInt64 = 12_000_000_000
...
final class AppState: ObservableObject {
    ...
    var effectiveRecordingContextLabel: String {
        recordingContextSelection.resolvedLabel(detected: detectedRecordingContext)
    }
    ...
    var canEnhanceWithFireflies: Bool {
        (mcp?.prefersMCP("fireflies") ?? false)
            && !transcript.isEmpty
            && status != .recording
            && !enhancingTranscript
            && !firefliesImporting
    ...
    private let transcriptionServiceFactory: (
        TranscriptionEngine, String, String, String
    ...
    init(transcriber: TranscriptionService? = nil,
         ...
         transcriptionServiceFactory: @escaping (
             TranscriptionEngine, String, String, String
         ) -> TranscriptionService = { engine, language, glossary, localModel in
    ...
    private func setUpCallDetection() {
        ...
        callNotifier.onNotificationsDenied = { [weak self] in
            self?.lastError = "Notifications are off for Cruxwing. Enable them in System Settings → Notifications to get call and meeting alerts."
        }
    ...
    func refineCurrentAnswer(_ kind: AnswerRefine.Kind) {
        ...
        Task { @MainActor in
            ...
            guard let refined = try? await trackingComputeUsage({
                ...
                lastError = "Couldn't refine the answer — the original is unchanged."
                return
            }
    ...
    func selectTranscriptionEngine(_ engine: TranscriptionEngine) -> Bool {
        ...
        guard available else {
            ...
            lastError = "\(engine.advantageTitle) is not available for this account. The active call is still using \(previousEngine.advantageTitle)."
            return false
        }
        ...
        // Capture setup owns the buffers until `.recording`; startRecording
        // consumes this pending choice immediately after capture comes online.
        guard status == .recording else { return true }
        ...
        guard switchActiveTranscriptionEngine(
            ...
            pendingEngineChange = nil
            lastError = "Couldn't switch this call to \(engine.advantageTitle). It is still using \(previousEngine.advantageTitle)."
            return false
        }
    ...
    func refreshLedger(quiet: Bool = false) async {
        ...
        guard let token = await wheesprAccessToken() else {
            if !quiet { lastError = "Sign in to see your Decision Ledger." }
            return
        }
        do {
            ...
            lastError = "Couldn't load the ledger: \(error.localizedDescription)"
        }
    }
    ...
    func attachContextFolder(url: URL) async {
        guard !contextImporting else {
            lastError = "Another context source is still indexing. Wait or cancel it first."
            return
        }
        ...
        guard replacesExisting || contextFolders.count < Self.maxAttachedContextFolders else {
            lastError = "You can attach up to \(Self.maxAttachedContextFolders) folders. Remove one before adding another."
            return
        }
        contextImporting = true
        defer { contextImporting = false }
    ...
    func rescanContextFolder(id: UUID) async {
        ...
        do {
            ...
            lastError = "Could not refresh \"\(displayName)\". Re-attach it if access changed."
        }
    }
    ...
    private func reportFolderSkips(_ folder: ContextFolder) {
        ...
        lastError = "\"\(folder.name)\": attached \(folder.files.count) file(s); "
            + "\(folder.skipped.count) left out (folder budget)."
    }
    ...
    func restoreContextFolders() async {
        ...
        persistContextFolders()
        if !lost.isEmpty {
            lastError = "Lost access to \(lost.joined(separator: ", ")) — re-attach to grant access again."
        }
    }
    ...
    func attachImages(_ urls: [URL]) async {
        ...
        if !errors.isEmpty { lastError = "Image issues — " + errors.joined(separator: "; ") }
    }
    ...
    func signInWithGoogleAccount(
        ...
        if authWorking {
            ...
            guard !authWorking else {
                lastError = "Sign-in is still finishing — check your browser window."
                return
            }
        }
        authWorking = true
    ...
    func applyMeetingContext(meetingID: String, title: String) async {
        ...
        do {
            ...
            lastError = "Couldn't load \(title): \(error.localizedDescription)"
        }
    }
    ...
    private func freshGoogleToken() async -> String? {
        ...
        guard Config.googleScopeVersion >= GoogleAuth.scopeVersion else {
            lastError = "Reconnect Google (Settings) to grant Docs & Sheets access."
            return nil
        }
        do {
            let tokens = try await GoogleAuth.validTokens(
    ...
    func exportAssistantAnswerToGoogleDocs() async {
        guard googleConnected else {
            lastError = "Connect Google in Settings to create a Doc."
            return
        }
        guard Config.googleScopeVersion >= GoogleAuth.scopeVersion,
              Config.googleGrantedServices.contains(GoogleService.docs.rawValue) else {
            lastError = "Reconnect Google (Settings) with Docs enabled to create a Doc."
            return
        }
        do {
            ...
            lastError = "Google Docs export failed: \(error.localizedDescription)"
        }
    }
    ...
    func exportAnswerTableToGoogleSheets(_ proposal: GoogleFileExport.Proposal) async {
        guard case let .createSpreadsheet(title, table) = proposal else {
            ...
            lastError = "That action can't be performed here."
            return
        }
        guard googleConnected else {
        ...
        do {
            ...
            lastError = "Couldn't create the spreadsheet: \(error.localizedDescription)"
        }
    }
    ...
    func undoLastSpreadsheetExport() async {
        ...
        do {
            ...
            lastError = "Couldn't move it to Trash: \(error.localizedDescription)"
        }
    }
    ...
    func exportAssistantAnswerToNotion() async {
        ...
        do {
            ...
            lastError = "Notion export failed: \(error.localizedDescription)"
        }
    }
    ...
    func importGoogleDoc(from urlString: String) async {
        guard let id = SourceURL.googleDocID(from: urlString) else {
            lastError = "That doesn't look like a Google Doc link."
            return
        }
        await withImporting {
    ...
    func importGoogleSheet(from urlString: String) async {
        guard let id = SourceURL.googleSheetID(from: urlString) else {
            lastError = "That doesn't look like a Google Sheet link."
            return
        }
        await withImporting {
            guard let token = await self.freshGoogleToken() else { return }
    ...
    private func appendContext(name: String, text: String) {
        guard !text.isEmpty else { lastError = "\(name) had no readable text."; return }
    ...
    func ask(_ text: String) {
        ...
        if !images.isEmpty, !Config.selectedModel.supportsVision {
            lastError = "The selected model doesn't support images — sending as text-only."
            images = []
        }
    ...
    private func exportAssistantAnswerAsWordFile() async {
        do {
            ...
            lastError = "Word export failed: \(error.localizedDescription)"
        }
    }
    ...
    private func toggleCallDictation(into text: Binding<String>) {
        ...
        guard !spoken.isEmpty else {
            lastError = "Nothing was transcribed in that window."
            return
        }
        appendToComposer(spoken, into: text)
    }
    ...
    private func run(prompt: String, images: [Data], skill: String? = nil, promptID: String? = nil) {
        ...
        var context = promptContext(
            query: prompt + "\n" + effectiveCallGoal)
        // Cross-meeting recall (roadmap F1): a "what did we decide about…"
        // prompt gets the prior-meeting record prepended, so the answer cites
        // saved sessions instead of relitigating from a blank context.
        if let recallBlock = DecisionRecallContext.block(for: prompt, store: sessionStore) {
            context = recallBlock + "\n\n" + context
        }
    ...
    private func startRecording() async {
        ...
        let sessionSettings = RecordingSettingsSnapshot.configured()
        let sessionEngine = sessionSettings.engine
        ...
        if sessionEngine == .deepgram, hasDeepgram {
            ...
            micChunker = AudioChunkBuffer(
                ...
                onChunk: liveChunkHandler(
                    ...
                    engine: sessionEngine, generationToken: generationToken,
                    routeLease: routeLease))
            // Record the remote (system) audio so it can be diarized at stop.
            if hasAssemblyAI, sessionSettings.assemblyDiarization {
                sessionAudioStart = Date()
                systemChunker?.onSamples = { [weak self] samples in
                    self?.sessionRecorder.append(samples)
                }
            }
        }
            ...
            if let requested = pendingEngineChange,
               ...
               let capturedEngine = activeSessionEngine {
                if switchActiveTranscriptionEngine(
                    ...
                    pendingEngineChange = nil
                    lastError = "Couldn't switch this call to \(requested.advantageTitle). It is still using \(capturedEngine.advantageTitle)."
                }
            } else {
                pendingEngineChange = nil
            }
    ...
    private func scheduleFirefliesEnhance() {
        ...
        firefliesEnhanceTask = Task { [weak self] in
            for delay in Self.firefliesEnhanceSchedule {
                ...
                guard self.currentSessionID == sessionID, !self.isRecording else { return }
    ...
    func enhanceTranscriptWithFirefliesNow() {
        ...
        cancelFirefliesEnhance()
        Task { await enhanceTranscriptWithFireflies(automatic: false) }
    }
    ...
    func importAndEnhanceWithFireflies() async {
        ...
        do {
            let fireflies = try await mcp.firefliesTranscript(near: recordingStartedAt ?? sessionDate)
            contextFiles.removeAll {
    ...
    private func enhanceTranscriptWithFireflies(automatic: Bool) async -> Bool {
        ...
        do {
            ...
            let fireflies = try await mcp.firefliesTranscript(
                near: recordingStartedAt ?? sessionDate,
    ...
    /// Replace the live remote-side transcript with AssemblyAI's diarized
    /// utterances, keep the local ("You") lines, and merge by time. Non-
    /// destructive: the existing transcript is kept if diarization fails.
    /// Re-transcribe the finished recording locally, in ONE pass.
    ///
    /// Measured across twelve real recordings, a single whole-file decode beat
    /// the live 6-second pipeline on every one of them — mean WER 0.2541 to
    /// 0.1731, a 32% cut — and did it at 0.12-0.16x realtime, so an hour-long
    /// call takes about eight minutes. `small` in one pass even beats
    /// `large-v3` chunked.
    ///
    /// The live path cuts at 6 seconds because a caption has to appear while
    /// people are still talking. Once the call is over that constraint is gone
    /// and the only reason to keep the slices is that nothing re-reads the
    /// audio — which it now does.
    ///
    /// Costs nothing and reveals nothing: no upload, no credits, no vendor, and
    /// the audio is already in memory for the diarize path, so this adds no
    /// retention.
    ...
    var canRetranscribeLocally: Bool {
        !localRetranscribing
            ...
            && !sessionRecorder.isEmpty
            && !isSessionLive
            // Only when the local engine is the one in use: re-running Whisper over
            // a call transcribed by Deepgram or the server would replace a
            // better transcript with a worse one.
    ...
    func retranscribeLocallyNow() {
        ...
        Task { await retranscribeSessionLocally() }
    }

    private func retranscribeSessionLocally() async {
        guard canRetranscribeLocally else { return }
        let wav = sessionRecorder.makeWAV()
        guard !wav.isEmpty, let start = sessionAudioStart else { return }
        let requestedSessionID = currentSessionID
        ...
        // NO glossary here, deliberately, and against the obvious instinct.
        //
        // In the LIVE chunked path a glossary is the largest accuracy lever
        // measured — term recall 0.53 to 0.93. In this whole-file pass it is a
        // bad trade: measured on the two IETF fixtures it bought 5 terms and
        // cost 14 points of RECALL, with the output falling from 467 words to
        // 372 against a 588-word reference. That is real speech disappearing,
        // and a fuller transcript is exactly what this pass exists to produce.
        //
        // The plausible mechanism is the same one that made a glossary hurt
        // large-v3: a decoder prompt competes with a long, self-consistent
        // context, and in one pass over a whole recording there is far more of
        // that context than in a six-second window.
        ...
        do {
            ...
            // A pass that produced nothing must not erase a transcript that
            // took the whole meeting to build.
            guard !cleaned.isEmpty else { return }

            // The glossary re-enters HERE, as text. The decode above runs
            // bare because prompt biasing was measured to delete speech on
            // every tier of the whole-file pass; a text restoration can fix
            // casing and repair non-word garbles, and can never make words
            // disappear. The user's terms may repair garbles; the shipped
            // lexicon is casing-only — fuzzy at 230 generic terms rewrote
            // out-of-lexicon acronyms into their one-edit neighbours.
            let restored = GlossaryRestore.restore(
                transcript: cleaned,
                glossary: Config.glossaryTerms,
                // PM default plus the call's topic-specific vocabulary, routed by
                // the theme already inferred from goal + transcript — so a legal /
                // engineering / fundraising call repairs its own jargon, not just
                // the ICP terms. Superset, restore-actionable, collision-audited.
                // …plus the VERTICAL pack this meeting signalled (F7): a
                // payments call gets KYC/PCI/interchange spelled correctly, a
                // clinical one gets SNOMED/FHIR, and a call that signals
                // neither carries neither — vocabulary that cannot appear can
                // only misfire. Routed off goal + transcript, casing-only.
                casingOnlyGlossary: DomainLexicon.casingOnlyTerms(
                    for: effectiveCallGoal + " " + cleaned)
                    + ThemeGlossary.terms(for: activeCallTheme))

            // Русская добавка поверх общей починки. Измерено на живой русской
            // речи: три движка разошлись на 29% терминов, и расходятся они не в
            // слухе, а в АЛФАВИТЕ — Whisper пишет «Prompt», Parakeet «Промпт».
            // Приведение к одному написанию подняло согласие с 71% до 89%.
            // Английские расшифровки не трогаются: проверка языка внутри.
            let localised = RussianLexicon.restoreIfRussian(restored)

            // Replaces the REMOTE side only, exactly as diarization does: the
            // mic track is this user's own speech and is already the better
            // recording of it.
            let mine = transcript.filter { $0.source == .mic }
            let rebuilt = TranscriptEntry(source: .system, text: localised,
                                          timestamp: start, speaker: nil)
            transcript = ([rebuilt] + mine).sorted { $0.timestamp < $1.timestamp }
            lastError = nil
            persistCurrentSession()
            devCallDiagnostics.record(event: "local_retranscribe",
                                      fields: ["chars": "\(restored.count)"])
        } catch {
            guard currentSessionID == requestedSessionID else { return }
            // Non-destructive: the existing transcript stands.
            lastError = error.localizedDescription
        }
    }

    /// On-device speaker labels for the lines already captured (roadmap F5).
    ///
    /// Runs against the recording this session already holds, so nothing is
    /// re-transcribed and nothing leaves the Mac. It labels the REMOTE track
    /// only — the mic track is this user, whose identity was never in doubt —
    /// and a line the diarizer says nothing about keeps the label it had, so a
    /// pass that fails halfway cannot degrade a transcript.
    ///
    /// Off by default (`LocalDiarization.isEnabled`): a confidently wrong
    ...
    func labelSpeakersLocally() async {
        ...
        guard LocalDiarization.canRun(sampleCount: samples.count) else { return }

        diarizing = true
        defer { diarizing = false }
        do {
            let segments = try await LocalDiarization.segments(samples: samples)
            guard currentSessionID == requestedSessionID, !segments.isEmpty else { return }
            transcript = SpeakerAssignment.apply(segments: segments, to: transcript,
                                                 sessionStart: start)
            persistCurrentSession()
            devCallDiagnostics.record(
                event: "local_diarization",
                fields: ["speakers": "\(SpeakerAssignment.distinctSpeakers(in: segments))",
                         "segments": "\(segments.count)"])
        } catch {
            // Silent: labels are a bonus on a transcript that already works,
            // and an error banner about them would be pure noise.
            Log.general.error("local diarization skipped — \(error.localizedDescription)")
        }
    }
        ...
        Task { [weak self, preparingTranscriber] in
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.lastError = "On-device transcription model couldn't load: \(error.localizedDescription) — check your connection, or switch the transcription engine in Settings."
                }
            }
        }
    }
    ...
    private func liveChunkHandler(
        ...
        { [weak self, transcriber] wav, _ in
    ...
    private func restoreEngineAfterFailedDeepgramHandoff(
        ...
        systemChunker.reconfigure(
            ...
            onSamples: sessionAudioStart != nil
        ...
        lastError = "Instant transcription couldn't start: \(message) Continued with \(settings.engine.advantageTitle)."
    }
    ...
    private func prepareTranscriberForRecording(engine: TranscriptionEngine,
        ...
        transcriber = transcriptionServiceFactory(
            engine, language, glossary, localModel)
        transcriberEngine = engine
        transcriberLocalModel = desiredLocalModel
        transcriberLanguage = language
    ...
    private func startDeepgram(
        ...
        let systemFallbackHandler: AudioChunkBuffer.ChunkHandler = { [weak self] wav, duration in
            let capturedAt = Date().addingTimeInterval(-duration)
            if preReadyBuffer?.capture(
                wav: wav, duration: duration, source: .system,
        ...
        let micFallbackHandler: AudioChunkBuffer.ChunkHandler = { [weak self] wav, duration in
            let capturedAt = Date().addingTimeInterval(-duration)
            if preReadyBuffer?.capture(
                wav: wav, duration: duration, source: .mic,
                capturedAt: capturedAt) == true { return }
            guard let transcriber = degrade.activeTranscriber() else { return }
            guard degradeRouteLease.begin() else { return }
            let generation = generationToken.read()
    ...
    private func degradeLiveStreamToLocal(message: String,
        ...
        let local = transcriptionServiceFactory(
            .local,
            activeSessionLanguage ?? Config.transcriptionLanguage,
        ...
        transcriberLocalModel = activeRecordingSettings?.localModel ?? Config.localWhisperModel
        transcriberLanguage = activeRecordingSettings?.language ?? Config.transcriptionLanguage
        transcriberGlossary = activeRecordingSettings?.glossary ?? Config.transcriptionGlossary
