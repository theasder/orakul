        ...
        if state.enhancingTranscript {
            ...
            HStack(spacing: Space.s) {
                ...
                if state.canRetranscribeLocally {
                    ...
                    .help("""
                    Повторно читает запись на этом компьютере перекрывающимися \
                    фрагментами. Ничего не уходит наружу. Живая расшифровка \
                    заменяется только если новый результат покрывает весь звонок \
