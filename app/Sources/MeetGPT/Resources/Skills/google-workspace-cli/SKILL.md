...
# Administers a live Google Workspace — Gmail, Drive, Calendar — through a CLI.
# The description matches ordinary meeting words ("Calendar scheduling", "Gmail
# automation"), and this app already holds the user's Google OAuth tokens, so it
# must never be pulled into a prompt by transcript similarity. Resolvable by id.
risk: critical
---
