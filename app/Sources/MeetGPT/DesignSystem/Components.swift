...
/// Заголовок раздела. Никогда не раздвигает контейнер.
///
/// Раздвигал. У боковой панели жёсткая ширина `.frame(width: 264)`, а такой
/// фрейм центрирует содержимое: стоило одному заголовку не поместиться, как весь
/// столбец становился шире панели и вылезал сразу за оба края. Обрезалась при
/// этом не только длинная строка — у каждой строки в панели пропадала первая
/// буква: «АСТРОЙКА» вместо «НАСТРОЙКА», «о-пилот» вместо «Ко-пилот».
///
/// Виновата была одна строка — «До понедельника звонков нет?»: в верхнем
/// регистре с трекингом это 233 pt при доступных 212. По-английски та же строка
/// занимала 171 pt и помещалась — под неё ширину 264 когда-то и выбрали.
///
/// Ограничения ниже делают такую поломку невозможной: строка сперва ужимается,
/// потом обрезается с многоточием, но лишней ширины не просит.
