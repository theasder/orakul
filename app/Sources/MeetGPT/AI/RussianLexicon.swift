...
public enum RussianLexicon {
    ...
    /// Починка ТОЛЬКО если расшифровка русская.
    ///
    /// Без этой проверки таблица вариантов сделала бы ровно то, за что её
    /// стоило бы выбросить: в английской расшифровке «prod» стал бы «прод», а
    /// «bug» — «баг». Направление кириллизации верно для русского созвона и
    /// неверно для любого другого, поэтому язык проверяется до, а не после.
    ///
    /// Порог по доле кириллицы, а не определитель языка: русская речь
    /// разработчика наполовину состоит из английских терминов, и определитель
    /// на таком тексте регулярно отвечает «английский».
